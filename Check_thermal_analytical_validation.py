...
def analytical_solution(X_coords):
    ...
    term1=erfc((coords-(gamma*t))/(2.*math.sqrt(t)))
    term2=(np.exp(gamma*coords))*erfc((coords+(gamma*t))/(2.*math.sqrt(t)))
    term3=1.+(0.5*gamma*(2.-coords+(gamma*t)))
    term4=erfc((2.-coords+(gamma*t))/(2.*math.sqrt(t)))
    term5=gamma*(math.sqrt(t/pi))*np.exp(-((2.-coords+(gamma*t))**2.)/(4.*t))

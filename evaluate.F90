module evaluate
    ...
    ! call defparam(symbol,value) or call defparam(symbol,expr)
    ! where:
    ! symbol(in) = String (Character (len = *))
    ! value (in) = input number (double, complex, real, ...)
    ! INSTEAD OF VALUE YOU CAN ALSO INTRODUCE AN EXPRESION:
    ! expr(in) = String (Character (len = *))
    interface defparam
    ...
    ! syntax is:
    ! expr(in) = String (Character (len = *))
    ! value(out) = output number(real, double, complex...)
    ! NOTE: BEFORE USING THIS METHOD YOU MAY WANT TO INTRODUCE THE VALUES OF THE
    ! VARIABLES, FOR THAT PURPOSE USE:
    !
    ! defparam(symbol,value)
    interface evalexpr
                ...
                case('F')  ! Token is a function name
                    ...
                    select case(trim(tempstr))      ! Evaluate function
                        ...
                        case('CONJG')
                            cval=conjg(oper1)
                        case('ANG')
                            cval=atan2(dble(aimag(oper1)),dble(real(oper1,kr8)))
                        case('REAL')
                            cval=real(oper1,kr8)
                        case('IMAG')
                            cval=aimag(oper1)

module semi_str_implicit
  ...
  Subroutine Semi_implicit(CFL, ndim, direct_solver, njac_its, time, nits,&
    ...
    do un_ele=1,totele_unst
        if ( meshList(un_ele)%region_id == 10 ) then
          tnew(:,:,un_ele) = 1.
        end if
      end do
    ...
    ! tnew(:,3:4) = 1.0 ! only correct for 1D?
    ...
    call make_sparse_matrix(sparse_lhs, totele_str*totele_unst, nloc)
...
!   print*, un_ele, str_ele, iface
!   print*, income(1)
!   print*, meshlist(un_ele)%Neig(mface), ele22, target_ele
!   print*,''
!   print*,''
!
! end if
...
! if ( itime==1 .and.its==1 ) then
!   print*, '398:target', target_ele, Npos, Nside
! end if
              if ( target_ele==0 .and. Npos==0 ) then ! it happens when ele is on the boundary
                ! target_ele = (un_ele-1)*4**n_split +str_ele ! gives 5 and 7
                ! target_ele = -(4**n_split)*(un_ele -1)
              elseif ( target_ele==0 .and. Npos/=0) then
                        if ( iface==1 ) then
                              ! Npos = meshList(un_ele)%Neig(mface)
                              ! Nside = meshList(un_ele)%fNeig(mface)
                              ipos = int(ipos/2)+1
                              target_ele = meshList(Npos)%s_ele(ipos, Nside)
                              ! target_ele = (Npos-1)*4**n_split + target_ele

                        elseif ( iface==2 ) then
                              ! Npos = meshList(un_ele)%Neig(mface)
                              ! Nside = meshList(un_ele)%fNeig(mface)
                              target_ele = meshList(Npos)%s_ele(2**n_split-irow+1, Nside)
                              ! target_ele = (Npos-1)*4**n_split + target_ele
...
!   print*, 'row ',irow
! end if
                              target_ele = meshList(Npos)%s_ele(irow, Nside)
! if ( itime==1 .and. its==1 ) then
!   print*, '426:target_ele ',target_ele, 'Npos,Nside', Npos,Nside
! end if
                        end if
...
! if ( itime==1 .and. its==1 ) then
!   print*, 'un_ele, str_ele',un_ele, str_ele
!   print*, 'iface,mface, income    ',iface, mface, income(1)
!   print*, 'Npos,Npos,Nside',Npos, Nside
!   print*, 'irow, ipos     ', irow, ipos
...
! if ( itime==1 .and. its==1 ) then
!   print*, un_ele, str_ele, target_ele, 'iface=', iface,mface, nside
!   print*, 'node',g_iloc, g_jloc, flux_ele
!   ! print*, snorm(1,idim),sdetwei(1),sn(1,iloc), '   ', snorm(2,idim),sdetwei(2),sn(2,iloc)
!   ! print*, (1.-income(1)),sn(1,jloc),usgi(1,idim), '   ', (1.-income(2)),sn(2,jloc),usgi(2,idim)
!   ! print*, income(1),sn2(1,jloc),usgi2(1,idim), '   ', income(2),sn2(2,jloc),usgi2(2,idim)
!   ! print*, flux_ele
!   print*, '----------------------------------------------------'
! end if
...
! if ( itime==1 .and. its==1 ) then
!   print*, '3 to 66 > 68:--------- 0', glob_flux(67,1:10)
!   print*, '3 to 66 > 68:---------10', glob_flux(67,11:20)
!   print*, '3 to 66 > 68:---------20', glob_flux(67,21:30)
!   print*, '3 to 66 > 68:---------30', glob_flux(67,31:40)
!   print*, '3 to 66 > 68:---------40', glob_flux(67,41:50)
!   print*, '3 to 66 > 68:---------50', glob_flux(67,51:60)
!   print*, '3 to 66 > 68:---------60', glob_flux(67,61:70)
!   print*, '3 to 66 > 68:---------70', glob_flux(67,71:80)
!   print*, '3 to 66 > 68:---------80', glob_flux(67,81:90)
!   print*, '3 to 66 > 68:---------90', glob_flux(67,91:96)
! end if

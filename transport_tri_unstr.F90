...
module transport_tri_unstr
  ...
  !brief> this is for tri-unstr
  ! implicit
  Subroutine unstr_implicit(CFL, ndim, direct_solver, njac_its, time, nits,&
    nface, dx, dy, nloc, snloc, ngi, sngi, n_s_list_no, u_x, u_y, with_stab, vtk_interval, cell_type)
    implicit none

    ! income variables
    real, intent(in):: CFL, dx, dy, time, u_x, u_y
    ...
    real:: length_x, length_y, mass_ele, stiff_ele, lhs
    real :: t1, t2, flux_ele
    ...
    real, allocatable :: rhs_array(:), glob_lhs(:,:), glob_mass(:,:), glob_flux_matrix(:,:)
    ...
    do itime=1,ntime
      ...
      glob_lhs = glob_lhs + glob_flux_matrix
    ...
    deallocate (face_ele,face_list_no,n,nlx,nx,M,nlx_lxx,nlxx,nlx_nod,weight,detwei,sdetwei,&
                ...
                mass_told,rhs_array,glob_lhs,glob_mass,glob_flux_matrix,glob_told_array,&
                inv_lhs,glob_flux,L1,L2,L3,L4,NLX_ALL)
  end subroutine unstr_implicit


  ! brief> this is for tri-unstr
  ! explicit
  Subroutine unstr_explicit(CFL, ndim, direct_solver, njac_its, time, nits,&
    nface, dx, dy, nloc, snloc, ngi, sngi, n_s_list_no, u_x, u_y, with_stab, vtk_interval, cell_type)
    implicit none
    ...
    integer :: gi, iface, totele, ele, ele2, ele22, n_split
    integer :: s_list_no, s_gi, iloc, jloc, i, ntime
    integer:: itime, idim, sndim, nonodes, mloc, col, row, row2
    integer :: errorflag, i_got_boundary, jac_its, its
    integer, allocatable :: face_ele(:,:), face_list_no(:,:)
    integer:: npoly, ele_type, ierr
    ...
    real :: t1_get_unstr_sn2, t2_get_unstr_sn2, time_get_unstr_sn2
    character(10) :: start, finish
    logical:: with_time_slab, D3
    ...
    ! xp(2): coordinates of nodes
    ! call ReadMSH(meshList,'./unstr_950272.msh',ierr, totnodes)
    ! call ReadMSH(meshList,'./98304_unele.msh',ierr, totnodes)
    ! call ReadMSH(meshList,'./untitled.msh',ierr, totnodes)
    ! call ReadMSH(meshList,'./semi_mesh.msh',ierr, totnodes)
    ! call ReadMSH(meshList,'./10_elements.msh',ierr, totnodes)
    ! call ReadMSH(meshList,'./4_elements.msh',ierr, totnodes)
    ...
    do ele=1,totele
      ...
      x_all(1,3,ele) = meshList(ele)%X(1,3)   ; x_all(2,3,ele) = meshList(ele)%X(2,3)
      ! initial condition
      if ( meshList(ele)%region_id == 12 ) then
        tnew(:,ele) = 1
      end if
    ...
    do itime=1,ntime
      ...
      do its=1,nits
        ...
        do ele=1,totele
          ...
          !Include the surface integral here:
          do iface = 1,nface
            ! call CPU_TIME(t1_getNeigDataMesh)
            call getNeigDataMesh(meshlist, ele, iface, Npos, Nside, lnodes2)
            ! call CPU_TIME(t2_getNeigDataMesh)
            ! time_getNeigDataMesh = t2_getNeigDataMesh - t1_getNeigDataMesh

            ! or it can be meshlist(ele)%Npos(iface) which needs to be defined first
            ele22 = Npos ! if ele22 is zero or negative assume on boundary
            i_got_boundary = (sign(1, -ele22) + 1 )/2 ! 1 means it is on the boundary
            r_got_boundary = real(i_got_boundary)
            ele2 = ele*i_got_boundary + ele22*(1-i_got_boundary)
            ! r_got_boundary=1.0 if we want to use the boundary conditions and have incomming velocity.
            ...
            sn   = face_sn(:,:,iface)             ! sn(sngi,nloc)
            snlx = face_snlx(:,:,:,iface)         ! slnlx(sngi, sndim, nloc)
            ! call CPU_TIME(t1_get_unstr_sn2)
            call get_unstr_sn2(face_sn2,nface,iface,Nside,sngi,nloc,snloc,SN_orig,SNLX_orig)
            ! call CPU_TIME(t2_get_unstr_sn2)
            ! time_get_unstr_sn2 = t2_get_unstr_sn2 - t1_get_unstr_sn2
            ...
            ! i_det_snlx_all = i_det_snlx_all + 1.
    ...
    ! print*, 'cpu finishing time = ', finish, '  starting time =  ',start

    ! print*, 'cpu time_getNeigDataMesh', time_getNeigDataMesh
    ! print*, ''
    ! print*, 'cpu_time tri_det_nlx = ', time_tri_det_nlx
    ! print*, 'no calling tri_det_nlx = ', i_tri_det_nlx
    ! print*, ''
    ! print*, 'cpu time_get_unstr_sn2', time_get_unstr_sn2
    ! print*, ''
    ! print*, 'cpu_time det_snlx_all = ', time_det_snlx_all
    ! print*, 'no calling det_snlx_all = ', i_det_snlx_all
    ! print*, ''


    deallocate(n, nlx, nx, M, nlx_lxx, nlxx, nlx_nod, weight, detwei, sdetwei,&
               sn,sn2,snlx, sweight, s_cont, tnew_loc, tnew_loc2, told, tnew, tnew_nonlin,&
               tnew_nonlin_loc, t_bc, u_bc, tnew_gi, tnew_xgi, tnew_sgi, tnew_sgi2, told_gi,&
               face_sn, face_sn2, face_snlx, u_loc, u_ele, u_loc2, x_loc, ugi,&
               x_all, xsgi, usgi, usgi2, income, snorm, norm,&
               ...
               L1, L2, L3, L4, NLX_ALL, meshList)
  end subroutine unstr_explicit

end module transport_tri_unstr
